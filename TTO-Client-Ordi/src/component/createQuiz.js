...
import { notification, List, Select,Layout, Menu, Icon, Tabs, Button, Breadcrumb, Modal, Checkbox, Input, Dropdown} from 'antd';
...
class CreateQuiz extends React.Component {
    ...
    sendNewQuiz = () => {
        ...
        console.log(newQuiz);
        if (newQuiz.name == null){
            notification.open({
                message: 'Entrer le nom ',
            });
            return null;
        }else if (newQuiz.topic == null) {
            notification.open({
                message: 'Choisir un theme ',
            });
        }else if (newQuiz.questions === []) {
            notification.open({
                    ...
                    <Content style={{ margin: '0 16px' }}>
                        ...
                        <div style={{ padding: 24, background: '#fff', minHeight: 480, display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style = {{ marginBottom:20, fontSize:16}}> <b>Thème</b>
                                <Select defaultValue="" value = {this.state.theme} style={{ width: 120, marginLeft: 50 }} onChange={this.onChangeTheme}>
